...
<script>
  ...
  import {getStatusDetails} from "../../config/globalUrl";
  ...
  export default {
    ...
    computed:{
      ...mapGetters(['lineSize','lines','statusDetails'])
    },
    methods: {
      ...mapActions(['setLineData','setAllDayData','setStatusDetails']),
      pixelCalc: function () {
      ...
      getData: function (thisMark, lines) {
        ...
        axiosPost(statusDetails).then(response => {
              if (response.data) {
                  if (response.data) {
                      this.setStatusDetails({data: response.data});
                  }
              }
          }).catch(err => {
