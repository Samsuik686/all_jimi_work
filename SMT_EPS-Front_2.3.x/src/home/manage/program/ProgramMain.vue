<template>
  <div class="program">
    ...
    <div v-if="isShow">
      <p class="tip">
        提醒：上传站位表时，若“未开始”的项目列表中存在“版面类型”、“工单号”、“线别”三者一致时，将覆盖上一份“未开始”中的项目
      </p>
      <el-form :inline="true" :model="programInfo" class="demo-form-inline">
        <el-form-item label="站位表">
          <el-input
            v-model.trim="programInfo.programName"
            placeholder="站位表"
          ></el-input>
        </el-form-item>
        <el-form-item label="工单">
          <el-input
            v-model.trim="programInfo.workOrder"
            placeholder="工单"
          ></el-input>
        </el-form-item>
        <el-form-item label="状态">
          <el-select
            v-model.trim="programInfo.state"
            placeholder="状态"
            value=""
          >
      ...
      <el-form :inline="true" :model="fileInfo" class="demo-form-inline">
        <el-form-item label="站位表文件">
          ...
          <el-input
            ...
            v-model="fileName"
            placeholder="请选择站位表上传"
          ></el-input>
        </el-form-item>
        <el-form-item label="版面">
          <el-select
            v-model.trim="fileInfo.boardType"
            placeholder="版面"
            value=""
          >
...
export default {
  ...
  methods: {
    ...
    upload: function () {
      //判断
      console.log(this.fileInfo.boardType);
    ...
    updateSmt: function () {
      let param = new FormData();
      let page = '';
      if(this.fileInfo.boardType == '0'){
        page = "C";
      }else if(this.fileInfo.boardType == '1'){
        page = "AB";
      }
      else if(this.fileInfo.boardType == '2'){
        page = "A";
      }
      else if(this.fileInfo.boardType == '3'){
        page = "B";
      }
      param.append("file", this.fileInfo.programFile);
      param.append("Page", page);
    
      let config = {
        headers: { "Content-Type": "multipart/form-data" },
      };
      axios.post(smtepsuploadExcel, param, config).then((response) => {
        if (response.data) {
