...
public class PCBSpinnerAdapter<T> extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (bean instanceof supplier_bean) {
            holder.name.setText(((supplier_bean)bean).getName());
        }else if (bean instanceof Pcbcompany_bean) {
            holder.name.setText(((Pcbcompany_bean)bean).getNickname());
        }
        else if (bean instanceof mission_bean) {
            holder.name.setText(((mission_bean)bean).getFileName());
        }
        return convertView;
    }
