   ...
   <Table ss:ExpandedColumnCount="${cosLenth}" ss:ExpandedRowCount="${size}" x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="80" ss:DefaultRowHeight="13.5">
		...
		<Row ss:StyleID="s50" ss:Height="44">
			<Cell ss:StyleID="s56" ss:MergeAcross="4">
				<Data ss:Type="String">初检故障管理列表</Data>
			</Cell>
		</Row>
	   ...
	   <#list peList as entity>
		   <Row ss:AutoFitHeight="0" ss:Height="23.1"> 
		    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.faultClass)!''}</Data></Cell>    <#-- === 故障类别 ==== -->
		    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.initheckFault)!''}</Data></Cell>	  	<#-- === 初检故障   ==== -->
		    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.description)!''}</Data></Cell>	  	<#-- === 故障现象描述  ==== -->
		    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.number)!''}</Data></Cell>       	    <#-- === 故障编码   ==== -->
		     	<#if entity.available??>
			     	<#if entity.available==0>									  	                      <#-- === 是否禁用  ==== -->
