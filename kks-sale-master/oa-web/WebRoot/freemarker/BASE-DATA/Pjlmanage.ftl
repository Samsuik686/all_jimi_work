 ...
 <Worksheet ss:Name="配件料管理列表">
  <Table ss:ExpandedColumnCount="${cosLenth}" ss:ExpandedRowCount="${size}" x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="80" ss:DefaultRowHeight="13.5">
	 ...
	 <Row ss:StyleID="s50" ss:Height="44">
			...
			<Data ss:Type="String">配件料管理列表</Data>
		</Cell>
	</Row>
   ...
   <#list peList as entity>
	   <Row ss:AutoFitHeight="0" ss:Height="24"> 
	    	...
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.proNO)!''}</Data></Cell>	  					<#-- === 编码   ==== -->
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.proName)!''}</Data></Cell>	  	    			<#-- === 名称  ==== -->
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.proSpeci)!''}</Data></Cell>       				<#-- === 规格   ==== -->
	    	...
	    	<#if entity.retailPrice??>																				<#-- === 零售价（￥）   ==== -->
			    <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.retailPrice)?string("#.##")}</Data></Cell>
		    <#else>  
			...
			<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.otherModel)!''}</Data></Cell>       			<#-- === 备选型号   ==== -->
