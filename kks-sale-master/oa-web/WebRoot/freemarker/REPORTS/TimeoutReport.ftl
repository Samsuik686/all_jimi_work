  ...
  <Table ss:ExpandedColumnCount="${cosLenth}" ss:ExpandedRowCount="${size}" x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="80" ss:DefaultRowHeight="13.5">
   ...
   <Row ss:StyleID="s50" ss:Height="44">
		<Cell ss:StyleID="s56" ss:MergeAcross="16">
			<Data ss:Type="String">超3天机器未寄出报表</Data>
		</Cell>
   </Row>
   ...
   <#list peList as entity>
	   <Row ss:AutoFitHeight="0" ss:Height="24">
	    	...
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.acceptanceTime)!''}</Data></Cell>   <#-- === 送修日期  ==== -->
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.timeoutBackTimeStr)!''}</Data></Cell>	  		<#-- === 应返回日期   ==== -->
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.sendPackTime)!''}</Data></Cell>	  		<#-- === 发送装箱日期   ==== -->
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.engineer)!''}</Data></Cell>	  		<#-- === 维修人   ==== -->

	    	<#if entity.timeoutDays??>
	    		<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.timeoutDays)}</Data></Cell>	  	    <#-- === 超/天==== -->
		    <#else>  
		  		<Cell ss:StyleID="s54"><Data ss:Type="String"></Data></Cell>
			</#if>
			
	    	<#if entity.state??>														<#-- === 状态描述  ==== -->
			   	<#if entity.state=='-1'>										  							
	    	...
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.timeoutReason)!''}</Data></Cell>	<#-- === 超期原因   ==== -->
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.dutyOfficer)!''}</Data></Cell>	  	<#-- === 责任人   ==== -->
		   <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.sendFicheckTimeStr)!''}</Data></Cell>	  	<#-- === 发送终检时间   ==== -->
		   <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.payTimeStr)!''}</Data></Cell>	  	<#-- === 客户付款日期   ==== -->
		   <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.sendPriceTimeStr)!''}</Data></Cell>	  	<#-- === 维修发送报价日期   ==== -->
		   <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.finChecker)!''}</Data></Cell>	  	<#-- === 终检人   ==== -->
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.timeoutRemark)!''}</Data></Cell>	<#-- === 备注==== -->
	   </Row>
    </#list>
