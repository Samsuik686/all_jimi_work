 ...
 <Worksheet ss:Name="销售配件收费报表">
   <Table ss:ExpandedColumnCount="${cosLenth}" ss:ExpandedRowCount="${size}" x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="80" ss:DefaultRowHeight="13.5">
		...
		<Row ss:StyleID="s50" ss:Height="44">
			<Cell ss:StyleID="s56" ss:MergeAcross="21">
				<Data ss:Type="String">销售配件收费报表</Data>
			</Cell>
		</Row>
	   ...
	   <#list peList as entity>
		  <Row ss:AutoFitHeight="0" ss:Height="24"> 
		    ...
		    <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.saleDate)!''}</Data></Cell>	  				<#-- === 销售日期 ==== -->
		    ...
		    <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.groupName)!''}</Data></Cell>   				<#-- === 组合名称  ==== -->
	        ...
	        <#else>  
		   	...
		   	<#if entity.payPrice??>
		   		<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.payPrice)?string("#.##")}</Data></Cell>	<#-- === 配件费 ==== -->
	   		<#else>  
			  	<Cell ss:StyleID="s54"><Data ss:Type="String"></Data></Cell>
		   	</#if>
		   	<#if entity.logCost??>
		   	...
		   	<#if entity.sumPrice??>
		   		<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.sumPrice)?string("#.##")}</Data></Cell>	<#-- === 总费用==== -->
	   		<#else>  
			  	<Cell ss:StyleID="s54"><Data ss:Type="String"></Data></Cell>
		   	</#if>
		   	<#if entity.receipt??>														
			   	<#if entity.receipt==0>										  									<#-- === 是否开发票 ==== -->					
				    <Cell ss:StyleID="s54"><Data ss:Type="String">是</Data></Cell> 
				<#elseif entity.receipt==1>										  						
		    ...
		    <#if entity.rate??>														
			   	<#if entity.rate==0>										  									<#-- === 税率 ==== -->					
				    <Cell ss:StyleID="s54"><Data ss:Type="String"></Data></Cell> 
				<#else>										  						
				    <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.rate)?string("#.##")}</Data></Cell> 
				</#if>    
			<#else>
				 <Cell ss:StyleID="s54"><Data ss:Type="String"></Data></Cell>
		    </#if>
		   	<#if entity.payMethod??>																			<#-- === 汇款方式  ==== -->
			   	<#if entity.payMethod=='personPay'>										  							
				    <Cell ss:StyleID="s54"><Data ss:Type="String">人工付款</Data></Cell> 
				<#elseif entity.payMethod=='aliPay'>										  						
				    <Cell ss:StyleID="s54"><Data ss:Type="String">支付宝付款</Data></Cell>     
			    <#elseif entity.payMethod=='weChatPay'>										  						
				    <Cell ss:StyleID="s54"><Data ss:Type="String">微信付款</Data></Cell>
				<#else> 
				    <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.payMethod)}</Data></Cell>    
			    </#if>
			<#else>
				 <Cell ss:StyleID="s54"><Data ss:Type="String"></Data></Cell>
		    </#if>
		   	...
		   	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.expressType)!''}</Data></Cell>				<#-- === 快递方式 ==== -->
		   	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.expressNO)!''}</Data></Cell>				<#-- === 快递单号 ==== -->
		   	<#if entity.payState??>														
			   	<#if entity.payState=='1'>										  								<#-- === 确认结果 ==== -->					
				    <Cell ss:StyleID="s54"><Data ss:Type="String">已确认</Data></Cell> 
				<#elseif entity.payState=='0'>										  						
				    <Cell ss:StyleID="s54"><Data ss:Type="String">未确认</Data></Cell>     
				<#else> 
				    <Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.payState)}</Data></Cell>    
			    </#if>
			<#else>
				 <Cell ss:StyleID="s54"><Data ss:Type="String"></Data></Cell>
		    </#if>
	    	<Cell ss:StyleID="s54"><Data ss:Type="String">${(entity.remark)!''}</Data></Cell>					<#-- === 备注原因==== -->
		   </Row>
	    </#list>
 </Table>
