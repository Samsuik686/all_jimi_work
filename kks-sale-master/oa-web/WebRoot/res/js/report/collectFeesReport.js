...
function queryListPage(pageNumber)  {
	...
	var url=ctx+"/collectFeesReport/collectFeesReportList";
	
	//页面只显示年月（日期<天>在sql语句中拼接成yyyy-MM-dd格式）
			...
			if (ret && ret.length > 0) {
				$.each(ret, function(i, item) {
					...
					if(i == ret.length-1){
						...
						item.payTimes="<label style='font-weight: bold;'>"+item.payTimes+"</label>";
					}
				});
			}
 ...
 */
function exportInfo(){
	ExportExcelDatas("collectFeesReport/ExportDatas?searchDate="+$("#searchDate").val());
}
