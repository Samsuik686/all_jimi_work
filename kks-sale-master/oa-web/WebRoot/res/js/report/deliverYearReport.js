...
$(function(){    
   datagridInit();  
   getCurrentYear();
   keySearch(queryDeliverYearList);
}); 
...
function datagridInit(){
	...
	queryDeliverYearList(1);
}
//--------------------------------------------- 初始化 》工具栏  ------------------------------------------ END

function queryDeliverYearList(pageNumber){
	currentPageNum=pageNumber;
	if(currentPageNum=="" || currentPageNum==null){
		currentPageNum=1;
	}
	var url=ctx+"/reportCon/getDeliverYearList";
	var param= null;
	if(!$('#dateTime').val()){
		getCurrentYear();
	}
...
function exportReportExcel(){
	...
	var dateTime = $("#dateTime").val();
	if(dateTime !=""){
		ExportExcelDatas("reportCon/deliverYearExport?dateTime=" + dateTime +
